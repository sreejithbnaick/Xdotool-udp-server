...
.PHONY: all
all: xdotool.1 libxdo.$(LIBSUFFIX) libxdo.$(VERLIBSUFFIX) server 
...
server.o: server.c xdotool.h xdo.h string_utils.c string_utils.h queue.h queue.c
...
server: LDFLAGS+=-Xlinker
ifneq ($(WITHOUT_RPATH_FIX),1)
server: LDFLAGS+=-rpath=$(INSTALLLIB)
endif
#xdotool: xdotool.o $(CMDOBJS) libxdo.$(LIBSUFFIX)
#	$(CC) -o $@ xdotool.o $(CMDOBJS) -L. -lxdo $(LDFLAGS)  -lm $(XDOTOOL_LIBS)
	
server: server.o xdotool.o $(CMDOBJS) libxdo.$(LIBSUFFIX)
	$(CC) -o $@ server.o xdotool.o $(CMDOBJS) -L. -lxdo $(LDFLAGS)  -lm $(XDOTOOL_LIBS)
